...
public class Player : MonoBehaviour {
	...
	public void Knockback(int strength) {
		...
		float direction = (transform.localScale.x > 0 ? -1 : 1) * 50;
		rb.AddForce(new Vector2(strength * direction, strength * 75));
	}
	...
	void Update() {
		...
		if (controller.moving.x != 0) {
			if (absVelX < maxVelocity.x) {
				forceX = standing ? speed * controller.moving.x : speed * controller.moving.x * airSpeedMult;
